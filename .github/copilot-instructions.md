...
- `source .venv/bin/activate && pytest tests/test_users.py::test_create_user -q`
...
- PostgreSQL server uses `@modelcontextprotocol/server-postgres` and prompts for `postgres_connection_string` at runtime; use the local default only if applicable: `postgresql://user_user:user_pass@localhost:5432/user_db`.
- Keep credentials out of committed files; use runtime prompts/environment variables instead of hardcoding secrets.

## High-Level Architecture

- The API is mounted under `settings.api_prefix` (default `/api/v1`) in `app/main.py`, and User endpoints are registered at `/users`, so effective routes are `/api/v1/users/...`.
- Request flow is strictly layered: `app/api/v1/routers/users.py` (HTTP boundary + query validation) → `app/modules/users/service.py` (business behavior and 404 decisions) → `app/modules/users/repository.py` (SQLAlchemy queries/commits).
- API boundary schemas live in `app/api/v1/schemas/users.py`, while internal module schemas and API mapping are handled by `app/modules/users/schemas.py` and `app/modules/users/mapper.py`.
...
- Preserve layering boundaries: route handlers should depend on `UserService`, and data access should stay in repositories (avoid direct DB session logic in routes).
...
- Keep response shaping in the service layer (`UserRead.model_validate(...)` and paginated envelope keys `items/total/limit/offset`) to match current API contracts.
- Schema normalization is part of the contract: `UserCreate`/`UserUpdate` validators trim strings and reject empty `first_name`/`last_name`, with basic email normalization and validation.
...
- Conditional (enable only when prompt intent is explicit): `repo-migration-safety`, `repo-doc-sync`.

Routing guidance:

- Prefer a core skill when both core and conditional skills appear relevant.
- Use conditional skills only when the user explicitly asks for migration safety/revision handling or documentation synchronization.
